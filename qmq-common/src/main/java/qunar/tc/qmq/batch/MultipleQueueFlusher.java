...
public class MultipleQueueFlusher<Item> {
    ...
    public MultipleQueueFlusher(String name, int queues, int capacityPerQueue, int batch, int flushInterval, int threads, LoadBalance<Item> loadBalance, Processor<Item> processor) {
        ...
        for (int i = 0; i < threads; ++i) {
            flushers[i] = new Flusher<>(capacityPerQueue, batch, flushInterval, processor);
        }

        for (int i = 0; i < queues; ++i) {
            final int flusherIndex = i % threads;
        ...
        for (int i = 0; i < threads; ++i) {
            ExecutorUtil.SHARED.submit(flushers[i]);
        }
    ...
    public boolean add(Item item) {
        final int index = loadBalance.idx(item);
    ...
    public void setBatch(int batch) {
        for (Queue<Item> flusherQueue : flusherQueues) {
            flusherQueue.setBatch(batch);
        }
    }
    ...
    private static class Flusher<Item> implements Runnable {
        ...
        public Queue<Item> create(int index, String name) {
            Queue<Item> queue = new Queue<>(name, index, capacityPerQueue, batch, flushInterval, notifier, processor);
    ...
    private static class Queue<Item> {
        ...
        Queue(String name, int index, int capacity, int batch, int flushInterval, Object notifier, Processor<Item> processor) {
            ...
            this.queue = new MpscLinkedQueue<>(capacity);

            Metrics.gauge("qmq." + name + ".queue").tag("i", String.valueOf(index)).call(new Gauge() {
                @Override
        ...
        public void flush() {
            ...
            List<Item> items = new ArrayList<>(batch);
            int size = queue.drainTo(items, batch);
            if (size > 0) {
                processor.process(new Group<>(index, items));
            }
        }
